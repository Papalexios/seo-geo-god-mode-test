...
// SOTA UPGRADE: Dynamic HTML Parser for Schema
function detectAndBuildDynamicSchema(htmlContent: string, articleUrl: string) {
    const schemas: any[] = [];
    
    // Auto-detect HowTo from <ol> with "Step"
    const stepMatches = [...htmlContent.matchAll(/<h[2-4][^>]*>(?:Step \d+:? )?(.*?)<\/h[2-4]>/gi)];
    // Simple heuristic: if we have "Step" keywords or just sequential headers in a list context
    // This regex is a bit greedy, let's use a simpler check for explicit HowTo structure
    const hasOrderedList = htmlContent.includes('<ol');
    
    if (hasOrderedList && stepMatches.length >= 3) {
        schemas.push({
            "@type": "HowTo",
            "@id": `${articleUrl}#howto`,
            "name": "How-To Guide", // Fallback name
            "step": stepMatches.slice(0, 8).map((m, i) => ({
                "@type": "HowToStep",
                "position": i + 1,
                "name": m[1] || `Step ${i+1}`,
                "text": m[1] || `Step ${i+1}`,
...
export function generateFullSchema(
    ...
    // 7. Dynamic HowTo/Other detection
    const dynamicSchemas = detectAndBuildDynamicSchema(content.content, articleUrl);
    if (dynamicSchemas.length > 0) schemas.push(...dynamicSchemas);
