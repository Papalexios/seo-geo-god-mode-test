...
const App = () => {
    ...
    const [apiKeys, setApiKeys] = useState(() => {
        ...
        let initialKeys;
        try { initialKeys = saved ? JSON.parse(saved) : defaults; } catch (e) { initialKeys = defaults; }
        if (initialKeys.geminiApiKey) delete initialKeys.geminiApiKey;
        return initialKeys;
    });
    ...
    const [geoTargeting, setGeoTargeting] = useState<ExpandedGeoTargeting>(() => {
        try { return JSON.parse(localStorage.getItem('geoTargeting') || '{"enabled":false,"location":"","region":"","country":"","postalCode":""}'); } catch(e) { return { enabled: false, location: '', region: '', country: '', postalCode: '' }; }
    });
    const [useGoogleSearch, setUseGoogleSearch] = useState(false);
    const [neuronConfig, setNeuronConfig] = useState<NeuronConfig>(() => {
        try { return JSON.parse(localStorage.getItem('neuronConfig') || '{"apiKey":"","projectId":"","enabled":false}'); } catch(e) { return { apiKey: '', projectId: '', enabled: false }; }
    });
    ...
    const [wpConfig, setWpConfig] = useState<WpConfig>(() => {
        try { return JSON.parse(localStorage.getItem('wpConfig') || '{"url":"","username":""}'); } catch(e) { return { url: '', username: '' }; }
    });
    ...
    const [siteInfo, setSiteInfo] = useState<SiteInfo>(() => {
        try { return JSON.parse(localStorage.getItem('siteInfo') || '{"orgName":"","orgUrl":"","logoUrl":"","orgSameAs":[],"authorName":"","authorUrl":"","authorSameAs":[]}'); } catch(e) { return { orgName: '', orgUrl: '', logoUrl: '', orgSameAs: [], authorName: '', authorUrl: '', authorSameAs: [] }; }
    });
    ...
    useEffect(() => {
        if (selectedItemForReview?.generatedContent) {
            setTimeout(() => { mermaid.run({ nodes: document.querySelectorAll('.mermaid') as any }); }, 500);
        }
    ...
    const bootstrapApp = () => {
        ...
        criticalKeys.forEach(key => {
            try { const data = localStorage.getItem(key); if (data) JSON.parse(data); } catch { localStorage.removeItem(key); }
        });
    };
    ...
    // GOD MODE: REAL ENGINE CONNECTION (NO SIMULATION)
    useEffect(() => {
        // Connect the engine's logs to the React State
        // @ts-ignore
    ...
    useEffect(() => {
        ...
        if (isGodMode) {
            // Force a check: Do we have pages?
            if (existingPages.length === 0 && !isCrawling) {
                setGodModeLogs(prev => ["⚠️ NO PAGES FOUND! Please go to 'Quick Refresh' and crawl your sitemap first.", ...prev]);
                return;
            }
    ...
    // ... (Validation, Sort, Filter helpers - same as original) ...
        ...
        try {
            ...
            switch (provider) {
                ...
                case 'anthropic':
                    ...
                    await callAiWithRetry(() => client.messages.create({ model: AI_MODELS.ANTHROPIC_HAIKU, max_tokens: 1, messages: [{ role: "user", content: "test" }], }));
                    isValid = true; break;
                 case 'openrouter':
                    client = new OpenAI({ baseURL: "https://openrouter.ai/api/v1", apiKey: key, dangerouslyAllowBrowser: true, defaultHeaders: { 'HTTP-Referer': window.location.href, 'X-Title': 'WP Content Optimizer Pro', } });
                    await callAiWithRetry(() => client.chat.completions.create({ model: 'google/gemini-2.5-flash', messages: [{ role: "user", content: "test" }], max_tokens: 1 }));
                    isValid = true; break;
                case 'groq':
                    client = new OpenAI({ baseURL: "https://api.groq.com/openai/v1", apiKey: key, dangerouslyAllowBrowser: true, });
                    await callAiWithRetry(() => client.chat.completions.create({ model: selectedGroqModel, messages: [{ role: "user", content: "test" }], max_tokens: 1 }));
                    isValid = true; break;
                 case 'serper':
                    const serperResponse = await fetch("https://google.serper.dev/search", { method: 'POST', headers: { 'X-API-KEY': key, 'Content-Type': 'application/json' }, body: JSON.stringify({ q: 'test' }) });
            ...
            if (isValid) {
                ...
                if (client) setApiClients(prev => ({ ...prev, [provider]: client as any }));
                setEditingApiKey(null);
            } else throw new Error("Validation check failed.");
        } catch (error: any) {
    ...
    useEffect(() => {
        Object.entries(apiKeys).forEach(([key, value]) => { if (value) validateApiKey(key.replace('ApiKey', ''), value as string); });
    }, []);
    ...
    const handleAnalyzeSelectedPages = async () => {
        ...
        if (!apiClients[selectedModel as keyof typeof apiClients]) {
            ...
            if (!fallback) { alert("No AI provider is connected. Please configure at least one API Key in Setup."); return; }
            if (!confirm(`The selected model '${selectedModel}' is not connected. Do you want to use '${fallback}' instead?`)) { return; }
        }
    ...
    const handlePlanRewrite = (page: SitemapPage) => {
        const newItem: ContentItem = { id: page.id, title: sanitizeTitle(page.title, page.slug), type: 'standard', originalUrl: page.id, status: 'idle', statusText: 'Ready to Rewrite', generatedContent: null, crawledContent: page.crawledContent, analysis: page.analysis };
    ...
    const handleRewriteSelected = () => {
        ...
        if (selectedPages.length === 0) { alert("Please select analyzed pages."); return; }
        const newItems: ContentItem[] = selectedPages.map(page => ({ id: page.id, title: sanitizeTitle(page.title, page.slug), type: 'standard', originalUrl: page.id, status: 'idle', statusText: 'Ready to Rewrite', generatedContent: null, crawledContent: page.crawledContent, analysis: page.analysis }));
        dispatch({ type: 'SET_ITEMS', payload: newItems });
        setSelectedHubPages(new Set());
        setActiveView('review');
    };
    const handleRefreshContent = async () => {
        if (!refreshUrl) { alert("Please enter a valid URL."); return; }
        setIsGenerating(true);
        const newItem: ContentItem = { id: refreshUrl, title: 'Refreshing...', type: 'refresh', originalUrl: refreshUrl, status: 'generating', statusText: 'Crawling...', generatedContent: null, crawledContent: null };
        dispatch({ type: 'SET_ITEMS', payload: [newItem] });
        setActiveView('review');
        try {
            ...
            const serviceCallAI = (promptKey: any, args: any[], format: 'json' | 'html' = 'json', grounding = false) => callAI(apiClients, selectedModel, geoTargeting, openrouterModels, selectedGroqModel, promptKey, args, format, grounding);
            const aiRepairer = (brokenText: string) => callAI(apiClients, 'gemini', { enabled: false, location: '', region: '', country: '', postalCode: '' }, [], '', 'json_repair', [brokenText], 'json');
            await generateContent.refreshItem({...newItem, crawledContent}, serviceCallAI, { dispatch, existingPages, siteInfo, wpConfig, geoTargeting, serperApiKey: apiKeys.serperApiKey, apiKeyStatus, apiClients, selectedModel, openrouterModels, selectedGroqModel, neuronConfig }, aiRepairer);
        } catch (error: any) { console.error("Refresh failed", error); dispatch({ type: 'UPDATE_STATUS', payload: { id: refreshUrl, status: 'error', statusText: error.message } }); } finally { setIsGenerating(false); }
    };
    const handleAnalyzeGaps = async () => {
        if (existingPages.length === 0 && !sitemapUrl) { alert("Please crawl a sitemap first."); return; }
        setIsAnalyzingGaps(true);
        try {
            const suggestions = await generateContent.analyzeContentGaps(existingPages, topic, (promptKey: any, args: any[], format: 'json' | 'html' = 'json', grounding = false) => callAI(apiClients, selectedModel, geoTargeting, openrouterModels, selectedGroqModel, promptKey, args, format, grounding), { dispatch, existingPages, siteInfo, wpConfig, geoTargeting, serperApiKey: apiKeys.serperApiKey, apiKeyStatus, apiClients, selectedModel, openrouterModels, selectedGroqModel, neuronConfig });
    ...
    const handleGenerateGapArticle = (suggestion: GapAnalysisSuggestion) => {
        const newItem: Partial<ContentItem> = { id: suggestion.keyword, title: suggestion.keyword, type: 'standard' };
        dispatch({ type: 'SET_ITEMS', payload: [newItem] });
        setActiveView('review');
    };
    const handleBulkRefreshAndPublish = async () => {
        const selectedPages = existingPages.filter(p => selectedHubPages.has(p.id));
        if (selectedPages.length === 0) { alert("Please select pages."); return; }
        if (!wpConfig.url || !wpConfig.username || !wpPassword) { alert("WP credentials missing."); return; }
        ...
        const addLog = (msg: string) => setBulkPublishLogs(prev => [ ...prev, `[${new Date().toLocaleTimeString()}] ${msg}`].slice(-50));
        const processItem = async (page: SitemapPage) => {
            addLog(`Processing: ${page.title || page.slug}...`);
            const item: ContentItem = { id: page.id, title: page.title || 'Untitled', type: 'refresh', originalUrl: page.id, status: 'generating', statusText: 'Initializing...', generatedContent: null, crawledContent: page.crawledContent };
            try {
                const serviceCallAI = (promptKey: any, args: any[], format: 'json' | 'html' = 'json', grounding = false) => callAI(apiClients, selectedModel, geoTargeting, openrouterModels, selectedGroqModel, promptKey, args, format, grounding);
                const aiRepairer = (brokenText: string) => callAI(apiClients, 'gemini', { enabled: false, location: '', region: '', country: '', postalCode: '' }, [], '', 'json_repair', [brokenText], 'json');
                let generatedResult: GeneratedContent | null = null;
                const localDispatch = (action: any) => { if (action.type === 'SET_CONTENT') generatedResult = action.payload.content; };
                await generateContent.refreshItem(item, serviceCallAI, { dispatch: localDispatch, existingPages, siteInfo, wpConfig, geoTargeting, serperApiKey: apiKeys.serperApiKey, apiKeyStatus, apiClients, selectedModel, openrouterModels, selectedGroqModel, neuronConfig }, aiRepairer);
                if (!generatedResult) throw new Error("AI failed.");
                addLog(`Generated. Publishing...`);
                const itemToPublish = { ...item, generatedContent: generatedResult };
                const result = await publishItemToWordPress(itemToPublish, wpPassword, 'publish', fetchWordPressWithRetry, wpConfig);
                if (result.success) addLog(`✅ SUCCESS: ${page.title}`); else throw new Error(result.message as string);
            } catch (error: any) { addLog(`❌ FAILED: ${page.title} - ${error.message}`); console.error(error); }
        };
        await processConcurrently(selectedPages, processItem, 1, (c, t) => setBulkAutoPublishProgress({ current: c, total: t }), () => false);
    ...
    const handleAddToRefreshQueue = () => {
        ...
        if (selected.length === 0) { alert("Please select pages."); return; }
        const newItems: ContentItem[] = selected.map(p => ({ id: p.id, title: p.title || 'Untitled', type: 'refresh', originalUrl: p.id, status: 'idle', statusText: 'Queued', generatedContent: null, crawledContent: p.crawledContent }));
        dispatch({ type: 'SET_ITEMS', payload: newItems });
        setActiveView('review');
    };
    const handleCrawlSitemap = async () => {
        if (!sitemapUrl) { setCrawlMessage('Enter URL.'); return; }
        setIsCrawling(true); setCrawlMessage(''); setExistingPages([]);
    ...
    const verifyWpEndpoint = useCallback(async () => {
        if (!wpConfig.url) { alert("Enter WP URL."); return; }
        setWpEndpointStatus('verifying');
        try {
            const response = await fetch(`${wpConfig.url.replace(/\/+$/, '')}/wp-json/`, { method: 'GET' });
            if (response.ok) setWpEndpointStatus('valid'); else setWpEndpointStatus('invalid');
        } catch (error) { setWpEndpointStatus('invalid'); }
    }, [wpConfig.url]);
    const handleGenerateClusterPlan = async () => {
        setIsGenerating(true); dispatch({ type: 'SET_ITEMS', payload: [] });
        try {
            ...
            const newItems: Partial<ContentItem>[] = [ { id: parsedJson.pillarTitle, title: parsedJson.pillarTitle, type: 'pillar' }, ...parsedJson.clusterTitles.map((cluster: { title: string }) => ({ id: cluster.title, title: cluster.title, type: 'cluster' })) ];
            dispatch({ type: 'SET_ITEMS', payload: newItems });
            setActiveView('review');
        } catch (error: any) { console.error("Error", error); } finally { setIsGenerating(false); }
    };
    const handleGenerateMultipleFromKeywords = () => {
        const keywords = primaryKeywords.split('\n').map(k => k.trim()).filter(Boolean);
        if (keywords.length === 0) return;
        const newItems: Partial<ContentItem>[] = keywords.map(keyword => ({ id: keyword, title: keyword, type: 'standard' }));
        dispatch({ type: 'SET_ITEMS', payload: newItems });
        setActiveView('review');
    };
    const handleGenerateImages = async () => {
        if (!apiClients.gemini && !apiClients.openai) { setImageGenerationError('Enter API key.'); return; }
        setIsGeneratingImages(true); setGeneratedImages([]); setImageGenerationError('');
        try {
            const imageService = async (prompt: string) => { const src = await generateImageWithFallback(apiClients, prompt); if (!src) throw new Error("Failed."); return src; };
        ...
        } catch (error: any) { setImageGenerationError(error.message); } finally { setIsGeneratingImages(false); }
    };
    ...
    const startGeneration = async (itemsToGenerate: ContentItem[]) => {
        setIsGenerating(true); setGenerationProgress({ current: 0, total: itemsToGenerate.length });
        const serviceCallAI = (promptKey: any, args: any[], format: 'json' | 'html' = 'json', grounding = false) => callAI(apiClients, selectedModel, geoTargeting, openrouterModels, selectedGroqModel, promptKey, args, format, grounding);
        const serviceGenerateImage = (prompt: string) => generateImageWithFallback(apiClients, prompt);
        await generateContent.generateItems(itemsToGenerate, serviceCallAI, serviceGenerateImage, { dispatch, existingPages, siteInfo, wpConfig, geoTargeting, serperApiKey: apiKeys.serperApiKey, apiKeyStatus, apiClients, selectedModel, openrouterModels, selectedGroqModel, neuronConfig }, (progress) => setGenerationProgress(progress), () => stopGenerationRef);
    ...
    const handleStopGeneration = (itemId: string | null = null) => {
        if (itemId) { stopGenerationRef.current.add(itemId); dispatch({ type: 'UPDATE_STATUS', payload: { id: itemId, status: 'idle', statusText: 'Stopped' } }); } 
        else { items.forEach(item => { if (item.status === 'generating') { stopGenerationRef.current.add(item.id); dispatch({ type: 'UPDATE_STATUS', payload: { id: item.id, status: 'idle', statusText: 'Stopped' } }); } }); setIsGenerating(false); }
    };
        ...
        <div className="app-container">
            ...
            <div className="main-layout">
                ...
                <main className="main-content">
                    {/* ... (Same UI structure, but ensure God Mode panel uses optimizedHistory) ... */}
                    {activeView === 'setup' && (
                                    ...
                                    <tbody>
                                        {items.map(item => (
                                            <tr key={item.id}>
                                                ...
                                                <td><button className="btn btn-small" onClick={() => setSelectedItemForReview(item)}>Review</button></td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
            ...
            {selectedItemForReview && (
                <ReviewModal 
                    ...
                    onPublishSuccess={() => {}}
                    publishItem={() => Promise.resolve({success:true, message: ''})}
                    callAI={() => Promise.resolve('')}
